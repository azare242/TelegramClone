...
const Navbar = () => {
  ...
  const [anchorElLANG, setAnchorElLANG] = React.useState<HTMLElement | null>(
    null
  );
  ...
  const handleProfileMenuOpenLANG = (
    event: React.MouseEvent<HTMLButtonElement>
  ) => {
    ...
    <>
      <AppBar color={`primary`}>
        <Toolbar sx={{ gap: '0.5rem' }}>
          ...
          {
            <>
              <IconButton onClick={handleProfileMenuOpenLANG}>
                <TranslateIcon />
              </IconButton>
              <Menu
