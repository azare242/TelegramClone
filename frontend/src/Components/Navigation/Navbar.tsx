    ...
    <>
      <AppBar color={`primary`}>
        <Toolbar sx={{ gap: "0.5rem" }}>
          ...
          {jsonWebToken && (
            <>
              <NavItem path="/a/" name={languageConfig.navbar.all} icon={<AppsTwoToneIcon/>}/>
              <NavItem path="/b/c" name={languageConfig.navbar.privates} icon={<PersonOutlineTwoToneIcon/>} />
              <NavItem path="/d/c" name={languageConfig.navbar.groups} icon={<GroupsTwoToneIcon/> }/>
              <IconButton sx={language === "FA" ? { marginRight: "auto" } : { marginLeft: "auto" } } onMouseEnter={() => console.log("focus in")} onMouseLeave={() => console.log("focus out")}>
